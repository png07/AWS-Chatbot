...
st.set_page_config(
    page_title="SM VITA",
    page_icon="🧑‍🎓",  # Temporary icon in case logo is missing
    layout="wide"
)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'claude' in chatbot_response.lower():
                chatbot_response = 'I am SM-VITA chatbot, a virtual assistant for SM VITA-related queries.'
...
if st.session_state.show_suggestions and not chat_session:
    ...
    cols = st.columns(len(suggested_questions) if len(suggested_questions) <= 3 else 3)
